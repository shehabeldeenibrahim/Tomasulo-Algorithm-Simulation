...
def main_fun(starting_point):
    ...
    data_memory,labels_array,instructions_memory,branch= parser(StartingPointInstMem,branch) 
    EndingPointInstMem  = findLastInstIndex(instructions_memory,StartingPointInstMem)
    ...
    while not(head==tail and isRobEmpty(rob) and cycles>2):
        ...
        flag,temp_PC,tail,miss = execute(reservation_s,data_memory,PC,rob,reg_stat,instruction_buffer,tail,miss)
        if(flag): 
            PC = temp_PC
    ...
    if(branch==0):
        ...
        misspred = miss/branch
    return "Number of cycles: " + str(cycles) + "\n" + "IPC:" + str(n_instructions/cycles) + "miss prediction:" +str(misspred)
